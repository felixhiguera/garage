...
//conexion with the database 
    ...
    if(!empty($_POST)){
            ...
            $topic =$_GET['id'];  
            $mechanic= mysqli_real_escape_string($conexion,$_POST['mechanic']);
            
            $sqlstatus = "UPDATE appointment SET idMechanic=$mechanic WHERE idAppointment = $topic" ;
            print_r($sqlstatus);
...
<div class="jumbotron">
    ...
    <form action="<?php $_SERVER["PHP_SELF"]?>" method="POST">
        <label for="exampleFormControlSelect1">Choose your mechanic</label>
        <select class="form-control" id="exampleFormControlSelect1" name="mechanic">
            <?php
                $query = mysqli_query($conexion, "SELECT * FROM mechanic");
                while($resultmechanic= mysqli_fetch_array($query)){
                    echo "<option value='".$resultmechanic['idMechanic']."'>".$resultmechanic['nameMechanic']."</option>";
                        }
            ?>
        </select>
